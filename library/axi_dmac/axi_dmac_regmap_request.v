...
wire [DMA_LENGTH_WIDTH+2-1:0] up_tlf_data;
wire up_tlf_valid;
wire up_tlf_write;
wire up_tlf_rd;
...
always @(*) begin
  ...
  9'h113: up_rdata <= up_tlf_data[DMA_LENGTH_WIDTH-1 : 0];   // Length
  9'h114: up_rdata <= up_tlf_data[DMA_LENGTH_WIDTH +: 2];  // ID
...
assign up_tlf_write = up_eot & response_partial & up_dma_enable_tlen_reporting;

always @(posedge clk) begin
  if (ctrl_enable == 1'b0) begin
    up_partial_length_valid <= 1'b0;
  end else begin
    if (up_tlf_write == 1'b1) begin
      up_partial_length_valid <= 1'b1;
    end else if (up_tlf_rd) begin
      up_partial_length_valid <= 1'b0;
